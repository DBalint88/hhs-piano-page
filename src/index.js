...
function printSongs() {
  ...
  for (let i = 1; i <= songs.length; i++) {
    ...
    levelValueHeader.textContent =
      "(" + determineSongValue(i) + " points each)";
  ...
  setTimeout(function () {
    try {
      ...
      if (typeof songList == "undefined") {
        songList = document.getElementsByClassName(
          "active-song-list-short-screen",
        )[0];
      }
...
async function countCurrentSongAttempts() {
  ...
  await getDocs(countQuery).then((snapshot) => {
    snapshot.docs.forEach((sub) => {
      userSubmissions.push(sub.id);
    });
  });
  console.log(
    "countCurrentSongAttempts 336 userSubmissions: ",
    userSubmissions,
  );
  let i = 1;
  while (true) {
    console.log(
      "countCurrentSongAttempts while loop.  Eval: " +
        userID +
        currentSongFbref +
        "(" +
        i +
...
async function updateQuotaDisplay() {
  ...
  await getDocs(quotaQuery).then((snapshot) => {
    snapshot.docs.forEach((sub) => {
...
function confirmInstructor() {
  modalParagraph.textContent =
    "One-time check: Who is your instructor for this class?";
...
async function submitSong(e) {
  ...
  if (pendingSongs.includes(currentSongFbref)) {
    if (
      confirm("Are you sure you want to unsubmit " + currentSongTitle + "?")
    ) {
      const docRef = doc(db, "userProfiles", userID);
    ...
    if (
      confirm("Are you sure you want to resubmit " + currentSongTitle + "?")
    ) {
      const docRef = doc(db, "userProfiles", userID);
...
async function createSubmission() {
  console.log(
    "createSubmission 484: Trying to create sub doc: ",
    userID + currentSongFbref + "(" + (currentSongAttempts + 1) + ")",
  );
  await setDoc(
    doc(
      db,
  ...
  currentSongAttempts = await countCurrentSongAttempts();
  console.log(
    "createSubmission says: currentSongAttempts = ",
    currentSongAttempts,
  );
  console.log(
    "submission sent successfully: ",
    userID + currentSongFbref + "(" + currentSongAttempts + ")",
  );
}
...
async function retractSubmission() {
  ...
  currentSongAttempts = await countCurrentSongAttempts();
  console.log(
    "retractSubmission says: currentSongAttempts = ",
    currentSongAttempts,
  );
...
async function updateUserLevel() {
  ...
  songs[userLevel - 1].forEach((element) =>
    allCurrentLevelSongs.push(element.id),
  );
